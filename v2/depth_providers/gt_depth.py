...
class MetricPngDepthProvider(DepthProvider):
    """Generic metric depth provider for PNG depth maps.

    depth_m = (png_value / png_max_value) * max_depth
    """
...
class IsaacSimDepthProvider(MetricPngDepthProvider):
    """IsaacSim GT depth provider (16-bit PNG -> metres)."""
...
class CODaDepthProvider(MetricPngDepthProvider):
    ...
    Default file naming matches ``depth000001.png``. Override
    ``filename_pattern`` if your CODa export uses a different format.
    """
...
class THUDSyntheticDepthProvider(DepthProvider):
    ...
    Raw uint16 depth values are returned as float32. Conversion to
    3-D points is handled by the dataset-specific unprojection formula.
    """
