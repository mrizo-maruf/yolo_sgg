...
def clean_pcd(
    ...
    """Statistical outlier removal + keep largest DBSCAN cluster.

    Parameters
    ----------
    pts : (N, 3) float32
        Raw world-frame points (e.g. from ``DepthProvider.get_masked_pcds``).

    Returns
    -------
    (M, 3) float32  —  cleaned subset, or empty (0, 3) if nothing survives.
    """
...
# ---------------------------------------------------------------------------
# Legacy: extract_points_from_mask (kept for backward compat)
# ---------------------------------------------------------------------------
...
def extract_points_from_mask(
    ...
    """Depth + binary mask -> cleaned (M,3) points in camera frame.

    .. note:: Prefer ``DepthProvider.get_masked_pcds`` + ``clean_pcd``
       in new code.  This function is kept for backward compatibility.
    """
...
def cam_to_world(points_cam: np.ndarray, T_w_c: np.ndarray) -> np.ndarray:
    """Apply 4x4 camera->world to (N,3) points."""
...
def bbox_visibility_fraction(
    ...
    zs = corners_cam[valid, 2]
    if np.all(zs > max_depth):
        return 0.0

    # Project to image
    us = corners_cam[valid, 0] / zs * intrinsics.fx + intrinsics.cx
    vs = corners_cam[valid, 1] / zs * intrinsics.fy + intrinsics.cy

    u_min, u_max = np.clip(us, 0, intrinsics.width).min(), np.clip(us, 0, intrinsics.width).max()
    v_min, v_max = np.clip(vs, 0, intrinsics.height).min(), np.clip(vs, 0, intrinsics.height).max()
    ...
    # Clip to image
    u_min_c = max(u_min, 0)
    u_max_c = min(u_max, intrinsics.width)
    v_min_c = max(v_min, 0)
    v_max_c = min(v_max, intrinsics.height)
    visible_area = max(0, u_max_c - u_min_c) * max(0, v_max_c - v_min_c)

    return visible_area / projected_area
